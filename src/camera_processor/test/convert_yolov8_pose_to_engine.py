...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Export format: 'engine'(TensorRT), 'onnx' or 'torchscript'.",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main() -> int:
    ...
    print(f"Exporting model to format='{args.format}' with settings -> imgsz={args.imgsz}, half={args.half}, device={args.device}")
    ...
    try:
        ...
        if args.format == "engine":
            ...
            print("ONNX export may require a compatible Ultralytics version and proper image size. Also verify PyTorch/Ultralytics install.")
        else:
            print("TorchScript export may require matching PyTorch versions.")
        print(f"Details: {exc}")
        return 1
