...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Pose Re-ID skeleton baseado em vídeo')
    ...
    parser.add_argument('--resize-width', type=int, default=0, help='Redimensionar largura (0 = manter; se ambos 0 aplica --scale)')
    parser.add_argument('--resize-height', type=int, default=0, help='Redimensionar altura (0 = manter; se ambos 0 aplica --scale)')
    parser.add_argument('--skip', type=int, default=0, help='Saltar N frames entre inferências (0 = nenhum)')
    parser.add_argument('--force-default', action='store_true', help='Ignora argumento vídeo e usa sempre o vídeo por omissão')
    parser.add_argument('--output', type=str, default='', help='Caminho de saída do vídeo processado (por omissão mesmo nome com _reid.mp4)')
    parser.add_argument('--codec', type=str, default='mp4v', help='FourCC para VideoWriter (ex: mp4v, XVID, avc1)')
    parser.add_argument('--no-display', action='store_true', help='Não mostrar janela, apenas guardar o vídeo')
    parser.add_argument('--scale', type=float, default=DEFAULT_SCALE, help='Fator de escala para reduzir tamanho (por omissão 0.5 = metade). Ignorado se usar resize-width/height.')
    ...
    parser.add_argument('--reid-threshold', type=float, default=DEFAULT_REID_THRESHOLD, help='Umbral de similaridade coseno para embeddings ReID (0-1).')
    parser.add_argument('--reappear-threshold', type=float, default=DEFAULT_REAPPEAR_THRESHOLD, help='Umbral de similaridade para reaparências de IDs velhos (0-1).')
...
def setup_video_io(args, video_path):
    ...
    if args.resize_height > 0:
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, args.resize_height)
    ...
    if args.resize_width > 0:
        out_width = args.resize_width
    if args.resize_height > 0:
        out_height = args.resize_height
    elif args.scale != 1.0 and args.scale > 0:
...
def process_frame(frame, model, reid_model, args, person_db: PersonDatabase, frame_index, counters, out_width, out_height):
    ...
    Args:
        ...
        next_person_id_ref: Referencia al próximo ID.
        frame_index: Índice del frame actual.
...
def main():
    ...
    cap, writer, out_width, out_height, fps_in, output_path = setup_video_io(args, video_path)
    ...
    try:
        while True:
            ...
            if not args.no_display:
                cv2.imshow('Skeleton-based Re-ID (Video)', annotated_frame)
